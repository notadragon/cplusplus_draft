...
%WORDINGNOTE: \textbf{ubdef:} \hfil except.handle.handler.ctor.dtor\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] referring to a member/base of an object from its ctor/dtor function-try-block handler.
%
...
%
%  \item[\textbf{implementations:}] Clang emits a compile-time warning for direct references in the handler, GCC does not, and there is no run-time or constant-evaluation detection.
...
\pnum
If the \grammarterm{noexcept-specifier} appears in a function type,
the \grammarterm{constant-expression} is the type's exception specification.
...
that is not potentially-throwing
can nevertheless exit via an exception
if, as part of that evaluation,
the violation of an implicit contract assertion\iref{basic.contract.general}
causes a call to the contract-violation handler\iref{basic.contract.handler}
and that handler exits via an exception.
\end{note}
\end{itemize}
...
\pnum
The exception specification for
\begin{itemize}
\item a comparison operator function\iref{over.binary} or
\item a postfix increment or decrement operator function\iref{over.inc.general}
\end{itemize}
