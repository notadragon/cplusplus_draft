...
\end{example}
%WORDINGNOTE: The above code handles destroying objects during stack
% unwinding, but it is not immediately obvious which actual automatic objects
% are destroyed by this process.  (All of them everywhere?   Probably not.)
% We want to be sure to include those created by postcondition captures, but
% it is not clear exactly how to accomplish that.
