...
\rSec1[basic.message]{Diagnostic Messages}%

\begin{bnf}
\nontermdef{diagnostic-message}\br
  unevaluated-string\br
  constant-expression
\end{bnf}

\pnum
A \grammarterm{diagnostic-message}
is a user-supplied string-like construct
for the purposes of specifying
a diagnostic message.
If a \grammarterm{diagnostic-message}
matches the syntactic requirements of \grammarterm{unevaluated-string},
it is an \grammarterm{unevaluated-string}
and the text of the \grammarterm{diagnostic-message}
is the text of the \grammarterm{unevaluated-string}.
Otherwise, a \grammarterm{diagnostic-message}
shall be
...
\end{itemize}
When the \grammarterm{diagnostic-message}
is not an unevaluated operand,
\begin{itemize}
\item
...
\item
the text of the \grammarterm{diagnostic-message} is formed by
the sequence of $N$ code units, starting at $D$, of
the ordinary literal encoding\iref{lex.charset}.
\end{itemize}
\begin{note}
An immediate invocation\iref{expr.const.imm}
that is a potentially-evaluated subexpressions\iref{intro.execution}
of the \grammarterm{diagnostic-message}
is evaluated at the point
where the \grammarterm{diagnostic-message} appers.
\end{note}
%WORDINGNOTE: The structure above is new, but the bullets have all been moved 
%  (with minimal changes) from [dcl.pre].
...
has an optionally supplied
\grammarterm{diagnostic-message}\iref{basic.message}.
The \grammarterm{diagnostic-message},
if any,
is evaluated at the point of the
...
should include
the text of the \grammarterm{diagnostic-message}
of the violated contract assertion,
if one is supplied.
