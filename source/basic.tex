...
\item
\grammarterm{identifier}
in a \grammarterm{postcondition-capture}
in a postcondition assertion\iref{dcl.contract.capture},
\item
\grammarterm{identifier}
...
\pnum
The locus of a \grammarterm{postcondition-capture}\iref{dcl.contract.capture}
is the start of the predicate
of the associated postcondition assertion.
...
that includes
\begin{itemize}
\item the predicate of $C$, and
\item the initializer of any postcondition-captures of $C$.
\end{itemize}
%WORDINGNOTE: The above bulletted list is broken down to avoid conflicts with
% P3400, which will introduce an expression as part of a contract assertion
% that is explicitly not part of the contract-assertion scope.
...
implicitly---upon
exit from the block,
completion of the full expression,
or completion of the postcondition assertion execution
for an object with
...
place, the behavior of the program is undefined\ubdef{original.type.implicit.destructor}.
This is true
even if the destruction occurs as part of stack unwinding.
\begin{example}
\begin{codeblock}
class T { };
struct B {
...
Postcondition captures also have automatic storage duration.
The storage for a postcondition capture lasts until
immediately after the execution of the associated postcondition
completes\iref{basic.contract.capture,basic.contract.eval}.

\pnum
If a variable with automatic storage duration has initialization or a destructor with side
effects, an implementation shall not destroy it except when specified
...
%WORDINGNOTE: Ths reads as "we will do the things we say we will do.
%  It is possible this paragraph should be made a note instead.
...
every precondition evaluation of the function call\iref{expr.call},
which in turn are sequenced before
every expression or statement
in the body of \placeholder{f},
which in turn are sequenced before
every postcondition evaluation of the function call.
...
\pnum
The \defnadj{execution of a}{contract assertion} $A$
consists of a (possibly empty) sequence of evaluations
that are sequenced in order.
The evaluations
are determined by the evaluation semantic
as described below.

\pnum
All of the evaluations
that are part of
an execution of a contract assertion
form a block
that is suspended
...
each object declared
by any postcondition captures
associated with the contract assertion.
\end{note}

\pnum
The execution of $A$ using the ignore semantic
consists of a single evaluation that has no effect.
\begin{note}
...
consists of
\begin{itemize}
\item the initialization of each postcondition capture of $A$, in order,
\item the evaluation of the predicate of $A$,
\item the occurrence of a contract violation based on the result of $A$, and
\item the destruction of each postcondition capture of $A$, in reverse order.
\end{itemize}
...
It is unspecified
whether the evaluations of the contract assertion
other than a potential contract violation are performed.
Let $B$ be the value of the predicate that would result from
performing the evaluations of the contract assertion.
\begin{note}
To determine whether a predicate would evaluate
to \keyword{true} or \keyword{false},
alternative evaluations
that produce the same result as the predicate
...
\pnum
There is an observable checkpoint\iref{intro.abstract} $O$
that happens before each of the evaluations of $A$
such that any other evaluation
that happens before that evaluation
also happens before $O$.

\pnum
A \defn{contract violation} of $A$ occurs when
\begin{itemize}
\item
$B$ is \keyword{false},
\item
...
Destruction of postcondition captures can result in
a contract violation even after
$B$ was determined to be \keyword{true}.
\end{note}
...
that are part of the current execution of that contract assertion
other than the destruction of postcondition captures that have already been initialized
are performed.
...
invoked from an evaluation of a function contract assertion\iref{dcl.contract.func}
exits via an exception,
...
\pnum
The \defn{evaluation sequence} of a list $R$ of contract assertions
is a list $S$ such that
\begin{itemize}
\item
