...
even though name lookup did not find the declaration of a template
(see \ref{temp.names}).
For example,
\begin{codeblock}
int h;
...
%WORDINGNOTE: \textbf{ubdef:} \hfil intro.object.implicit.create\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] using storage in a way no choice of implicitly-created object could make well-defined.
%
%  \item[\textbf{runtime-checkable:}] Not generally, requires predicting whether the object will be used correctly as a given type
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] None
%
...
%WORDINGNOTE: \textbf{ubdef:} \hfil intro.object.implicit.pointer\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] producing a pointer to an implicitly-created object when no pointer value into that storage is well-defined.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether storage can hold implicit lifetime objects
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.align.object.alignment\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] placing an object at a storage address misaligned for its type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert alignment check when creating or reading objects.
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] UBSan's \tcode{alignment} check (stock and routed through the handler); and, in the P3850 prototype, a native middle-end check on both compilers.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.pointer.delete\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{delete}-ing a pointer to an object whose lifetime has already ended.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by constant evaluation on Clang.
%  \end{ubsummaryitemize}
\item
  the pointer is used to access a non-static data member or call a
  non-static member function of the object\ubdef{lifetime.outside.pointer.member}, or
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.pointer.member\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] accessing a member through a pointer to an object outside its lifetime.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by constant evaluation on both GCC and Clang.
%  \end{ubsummaryitemize}
\item
...
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.pointer.virtual\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] converting a pointer outside its lifetime into a virtual base class pointer.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] heuristically caught by UBSan's \tcode{vptr}
%  check (detection relies on the ended object's vtable still being recognizably
%  invalid) and exhaustively caught by constant evaluation, on both GCC and Clang.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.pointer.dynamic.cast\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{dynamic_cast} through a pointer to an object outside its lifetime.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] heuristically caught by UBSan's \tcode{vptr} check on Clang
%  (detection relies on the ended object's vtable) and rejected in constant evaluation on Clang; GCC has no detection channel (it does not instrument \tcode{dynamic_cast} for \tcode{vptr}, and its constant evaluator accepts the cast).
%  \end{ubsummaryitemize}
\end{itemize}
\begin{example}
\begin{codeblock}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.glvalue.access\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] using a glvalue to read or write an object outside its lifetime (before construction completes or after it ends).
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] heuristically caught by AddressSanitizer on both compilers (default heap-use-after-free; extra flags for stack/overflow variants) and exhaustively caught by constant evaluation.
%  \end{ubsummaryitemize}
\item the glvalue is used to call a non-static member function of the object\ubdef{lifetime.outside.glvalue.member}, or
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.glvalue.member\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] calling a non-static member function through a glvalue outside its lifetime.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.glvalue.virtual\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] binding a glvalue outside its lifetime to a virtual base class reference.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%WORDINGNOTE: \textbf{ubdef:} \hfil lifetime.outside.glvalue.dynamic.cast\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] using a glvalue to a dead object as the operand of \tcode{dynamic_cast} or \tcode{typeid}.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] Clang heuristically catches it at run time via UBSan's \tcode{vptr} check (both \tcode{noexcept}-enforce and \tcode{noexcept}-observe; detection relies on the ended object's vtable); GCC has no run-time channel.  Both compilers reject it in constant evaluation, though only incidentally -- the virtual base makes the class ineligible for a constexpr constructor/destructor, so evaluation fails before the offending \tcode{dynamic_cast} is reached.
%  \end{ubsummaryitemize}
\end{itemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil original.type.implicit.destructor\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] an implicit destructor at scope exit running on storage reused for a different type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Checking this would require maintaining type and lifetime information
...
%WORDINGNOTE: \textbf{ubdef:} \hfil creating.within.const.complete.obj\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] creating a new object in storage a \tcode{const} complete object
%  occupies or used to occupy.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether storage is associated with a \tcode{const} object
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] Clang's constant evaluation catches both \tcode{.live} and
%  \tcode{.reuse} subsets.
%  \end{ubsummaryitemize}
\begin{example}
\begin{codeblock}
...
produces an erroneous or indeterminate value,
then the value produced by that operator is
erroneous or indeterminate respectively.
Except in the following cases,
all evaluations have an implicit precondition assertion
that they will not produce an indeterminate value\ubdef{basic.indet.value},
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.indet.value\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] reading an uninitialized (indeterminate) value.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether storage has been initialized
%
%  \item[\textbf{replacement:}] Only for built-in types: initialise default-initialised variables with erroneous value
%
%  \item[\textbf{implementations:}] caught by MemorySanitizer on Clang; also
%  rejected during constant evaluation.
%  \end{ubsummaryitemize}
and if they do then the result of the evaluation is
...
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.stc.alloc.dealloc.constraint\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] a user-replaced allocation or deallocation function violating its
%  semantic constraints.
%
%  \item[\textbf{runtime-checkable:}] Partially
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Add postcondition assertions to allocation functions, evaluate them.
%  Some constraints can be expressed this way, some (such as global distinctness) are not
%  locally verifiable.
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] Some caught by UBSan's \tcode{undefined} check on Clang.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.stc.alloc.zero.dereference\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] dereferencing the non-null pointer returned by a zero-size
%  allocation
%
%  \item[\textbf{runtime-checkable:}] Yes
%
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by AddressSanitizer on both compilers.
%  \end{ubsummaryitemize}
\begin{footnote}
The intent is
...
\impldef{subset of values of a trivially copyable type
         to which a value representation corresponds}
subset of this set.
...
the behavior is undefined\ubdef{basic.compound.pointer.before.storage.duration}.
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.compound.pointer.before.storage.duration\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] forming a pointer to an object before its storage duration begins.
%
%  \item[\textbf{runtime-checkable:}] No, requires knowing that undefined behavior will or
%  will not be encountered with any potential value a pointer might have.
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Would require properly identifying when angelic provenance is unable
%  to apply.
%
%  \item[\textbf{replacement:}] None
%
...
\begin{itemize}
\item If $E$ is an indirection\iref{expr.unary.op},
the behavior is undefined\ubdef{basic.compound.invalid.pointer}.
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.compound.invalid.pointer\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] dereferencing a pointer to storage that has already been freed.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track object lifetimes across the entire program
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] heuristically caught by AddressSanitizer,
%  on both compilers; also
%  rejected during constant evaluation on both compilers.
%  \end{ubsummaryitemize}
\item If $E$ either
\begin{itemize}
\item is a boolean conversion\iref{conv.bool} or
\item is performed by
a unary \tcode{+}, additive, three-way comparison, relational, or equality
operator\iref{expr.compound},
\end{itemize}
the behavior is \impldef{invalid pointer value in the context of an evaluation}.
\end{itemize}
\begin{note}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil intro.execution.unsequenced.modification\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] unsequenced side effects (or a side effect and a read) on the same scalar object, single-threaded.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Identify all potential read operations that are not sequenced with respect to each given write operation; insert checks to identify if those operations are referencing the same address
%
%  \item[\textbf{replacement:}] Sequence operations in some unspecified order
%
%  \item[\textbf{note:}] Needs more careful specification work, defer to future paper.
%
%  \item[\textbf{implementations:}] None
%  \end{ubsummaryitemize}
\begin{itemize}
...
every precondition assertion of the function call\iref{expr.call},
...
%WORDINGNOTE: \textbf{ubdef:} \hfil intro.races.data\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] unsynchronized, potentially concurrent accesses to the same memory location, at least one a write.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track from which threads memory is accessed and when accesses synchronize with each other; only practical for a subset of cases
%
%  \item[\textbf{replacement:}] Could possibly make all primitive memory accesses implicitly atomic, but that is neither specified nor ready to propose
%
%  \item[\textbf{implementations:}] heuristically detected by ThreadSanitizer on both compilers.
%  \end{ubsummaryitemize}
\begin{note}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil intro.progress.stops\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] a non-terminated thread stops making progress -- e.g., a busy-wait whose loop condition is not a constant expression (a trivial infinite loop is not undefined).
%
%  \item[\textbf{runtime-checkable:}] No, requires (quite literally) solving the halting problem
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] No checking strategy exists as whether a thread will make progress is undecidable
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.start.main.exit.during.destruction\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] calling \tcode{std::exit} recursively during the destruction sequence
%  it would restart.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No, needs new instrumentation of existing functionality
%
%  \item[\textbf{checking:}] Track whether static or thread-local objects are currently being destroyed
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
%WORDINGNOTE: \textbf{ubdef:} \hfil basic.start.term.use.after.destruction\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] using a static-storage-duration object after its destructor has
%  run.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track the lifetime of static objects
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
\defnadj{Explicit}{contract assertions} are introduced by
...
\defnadj{Implicit}{contract assertions} are introduced
as parts of other language constructs.

\pnum
Each explicit contract assertion
has a \defnadjx{contract-assertion}{predicate}{predicate},
which is an expression of type \tcode{bool}.

\pnum
Each implicit contract assertion
has a \defnadjx{contract-assertion}{condition}{condition}
under which it is considered satisfied.

\begin{note}
If it is determined
during program execution
that the predicate does not evaluate
to \tcode{true},
or that the condition of an implicit contract assertion
indicates that the contract assertion is not satisfied,
...
uses one of the following five \defn{evaluation semantics}:
\defnx{ignore}{contract evaluation semantics!ignore},
\defnx{observe}{contract evaluation semantics!observe},
\defnx{enforce}{contract evaluation semantics!enforce},
\defnx{quick-enforce}{contract evaluation semantics!quick-enforce}, or
\defnx{assume}{contract evaluation semantics!assume}.
...
and need not allow all five evaluation semantics as possibilities.
...
\pnum
If the predicate of
a contract assertion evaluated with the assume semantic
would not evaluate to \tcode{true},
or the condition of such a contract assertion
is not satisfied,
the evaluation has unconstrained behavior\iref{intro.abstract};
otherwise the evaluation has no effect.

\pnum
\begin{note}
If a contract assertion has a predicate,
and that contract assertion is
evaluated with the ignore or assume semantic,
the predicate
...
\begin{itemize}
\item
the contract assertion has a condition
...
or the contract assertion has a condition
that is satisfied,
...
\rSec2[basic.contract.implicit]{Implicit contract assertions}

\pnum
A built-in operation $O$
may have an
\defnadj{implicit}{precondition assertion} $C$ applied to it.
If so,
the evaluation in sequence of $C$
is sequenced before
the evaluation of $O$ and
after the evaluation of
all operands of $O$.

\pnum
All undefined behavior has a
\defnadj{guarding}{contract assertion}
that is an implicit contract assertion
whose condition is never satisfied.

\pnum
\begin{note}
If the guarding contract assertion
of undefined behavior
is
\begin{itemize}
\item evaluated with the ignore semantic,
\item evaluated with the assume semantic, or
\item evaluated with the observe semantic
and the contract-violation handler returns normally
\end{itemize}
then the behavior is unconstrained\iref{intro.abstract}.
\end{note}

\pnum
\begin{note}
Behavior with
an implicit precondition assertion that it does not occur
results in unconstrained behavior
when that assertion is evaluated with the
assume semantic\iref{basic.contract.eval}
but in all other cases has specified behavior.
\end{note}

%WORDINGNOTE: The above note's purpose is to  make it clear that the places where we
% have replaced what was previously undefined behavior with
% erroneous behavior that has an implicit precondition can be
% implemented in a conforming manner by current compilers by
% dictating that the associated guarding contract assertion is
