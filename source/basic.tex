...
that includes the predicate of $C$.
...
\rSec2[basic.scope.contract.control]{Assertion-control scope}%

\pnum
Each
\grammarterm{assertion-control-specifier}\iref{basic.contract.control}
and
\grammarterm{assertion-control-expression}\iref{expr.contract.control}
introduces an \defnadj{assertion-control}{scope}
that includes
its \grammarterm{constant-expression}.

\begin{note}
Within an assertion-control scope
name lookup will search additional namespaces
introduced by
\grammarterm{using-directives}
with the \keyword{contract_control} keyword\iref{namespace.udir,basic.lookup}.
\end{note}
...
\pnum
A \grammarterm{using-directive} $U$ is
\term{active} in a scope $S$ at a program point $P$
\begin{itemize}
\item if
$U$ precedes $P$ and inhabits either $S$ or
the scope of a namespace nominated by a \grammarterm{using-directive}
that is active in $S$ at $P$, and
\item if $U$ has the \keyword{contract_control} keyword
then $P$ is within an assertion-control scope.
\end{itemize}
...
\rSec2[basic.contract.control]{Control}

\indextext{contract assertion!control}%

\begin{bnf}
\nontermdef{assertion-control-specifier}\br
    \terminal{<} constant-expression \terminal{>}
\end{bnf}

%NOTE: Multiple strategies were considered for how to specify facets:
%
%  1. Define the concepts in the standard library and have the core wording
%     reference them by name, saying "if the assertion control object models
%     std;:contracts::labels::..." to identify when the assertion control
%     object participates in a particular facet.
%
%  2. Define facet as a first-order term and describe the facets, and define
%     what it means to participate in a facet.  Standard library concepts will
%     refer back to that definition.
%
%  3. Define facets as a first-order term and include the satisfaction of the
%     Standard library concept along with that definition, ensuring that we
%     retain consistency between the core and library behavior.
%
%  4. Define new adjectives for each facet and do not use the term facet in the
%     Standard.  Say things along the lines of "If the assertion control object
%     is an allowed semantics control object..."   Standard library concepts
%     refer back to these definitions.
%
%  5. Define the adjectives and include modeling the concept in that definition
%     keeping the library dependency but in only one place.
%
%  For now, we are going with option 4 for certain reasons:
%
%  1. This keeps the core wording from having to depend on the Standard library
%     definitions, especially when
%
%  2. We can keep the library concept definitions sufficiently in synch as we
%     update the Standard, we do not have to bake in that requirement.
%
%  3. We don't need to deliberate on introducing "facet" as a new term of art.
%     If we change our mind on this last one, we could conceivably reuse it
%     for things like various aspects of the coroutine promise object.

\pnum
The \grammarterm{constant-expression}
is parsed as a \grammarterm{template-argument-list}\iref{temp.names}.
\begin{note}
A \tcode{>} in the \grammarterm{constant-expression}
is treated as the closing delimeter, and
a \tcode{>>}
is treated as
two consecutive but distinct
\tcode{>} tokens.
\end{note}

\pnum
A type is an \defnadj{assertion}{control type} if the search for the name
\tcode{assertion_control_object} in its scope finds a declaration of a type.

\pnum
Every contract assertion $C$ may have associated with it
an \defn{assertion control object}
that is a constexpr variable.
\begin{itemize}
\item
If $C$ has
an \grammarterm{assertion-control-specifier} $S$
then it has an assertion control object $O$
whose type is deduced from $S$
and which is initialized by $S$.
The type of $O$ must be an assertion-control type.
\item
Otherwise, the contract assertion
has no assertion-control object.
\end{itemize}

%NOTE:We are avoiding having a default assertion-control object because
%to do so we would have to define all of the properties that its unspecified
%type does not have.


\pnum
\begin{note}
$S$ is manifestly constant-evaluated\iref{expr.const.defns},
$O$ must have a literal type\iref{basic.types.general}, and
$S$ is not part of the immediate context\iref{temp.deduct.general}.
\end{note}

\pnum
A type is an \defnadj{allowed-semantics}{control type} if
\begin{itemize}
\item it is an assertion-control type, and
\item it has a static member named \tcode{allowed_semantics} that is
\begin{itemize}
\item usable in constant expressions, and
\item can be converted to the type \tcode{std::contracts::labels::evaluation_semantic_set}.
\end{itemize}
\end{itemize}

\pnum
A type \placeholder{T} is an \defnadj{computed-semantics}{control type} if
\begin{itemize}
\item it is an assertion-control type, and
\item the expression \tcode{o.compute_semantic(s)}
has the type \tcode{std::contracts::evaluation_semantic}
where \tcode{o} is an object of type \tcode{T}
and \tcode{s} is a prvalue of type \tcode{std::contracts::evaluation_semantic}.
\end{itemize}

\pnum
A type \placeholder{T} is a
\defnadj{local-violation}{control type} if
\begin{itemize}
\item it is an assertion-control type, and
\item the expression \tcode{o.handle_contract_violation(v)} is valid
where \tcode{o} is an object of type \placeholder{T}
and \tcode{v} is a \tcode{const} lvalue of
type \tcode{std::contracts::contract_violation}
and its type is \tcode{void}
or is convertible to \tcode{std::contracts::violation_handled}\iref{support.contract.control.vhandled}.
\end{itemize}
...
\pnum
A contract evaluation semantic is an
\defnadj{allowed}{contract evaluation semantic}
of a contract assertion $C$
if
\begin{itemize}
\item $C$ does not have an assertion-control object,
\item the assertion-control object $O$ of $C$ is not
an allowed-semantics control type\iref{basic.contract.control}, or
\item the expression \tcode{o.allowed_semantics.contains(S)}
is \tcode{true}
where \tcode{S} is the corresponding enumerator for the evaluation semantic
in \tcode{std::contracts::evaluation_semantic}.
\end{itemize}

\pnum
If a contract assertion has no allowed evaluation semantics
the program is ill-formed.
The \defnadj{configured}{contract evaluation semantic}
for an evaluation of a contract assertion
is an allowed contract evaluation semantic
chosen in an
...
configured evaluation semantics depends on the implementation
...
\pnum
The \defnadj{effective}{contract evaluation semantic}
for an evaluation $E$ of a contract assertion $C$ is
\begin{itemize}
\item the configured contract evaluation semantic
of $E$ if
\begin{itemize}
\item $C$ has no assertion-control object, or
\item if the assertion control object $O$ of $C$
is not a computed-semantics control type\iref{basic.contract.control},
\end{itemize}
or
\item the contract evaluation semantic corresponding
to the value of the expression \tcode{o.compute_semantic(S)}
where \tcode{o} is the assertion control object of $C$ and
\tcode{S} is the enumerator corresponding to
the configured contract evaluation semantic of $E$.
\end{itemize}
The effective contract evaluation semantic
shall be an allowed contract evaluation semantic.
...
then the contract-violation handling occurs.

\pnum
Contract-violation handling consists of a number of steps evaluated in order.
\begin{itemize}
\item An object \tcode{V}
of type \tcode{std::contracts::contract_violation}\iref{support.contract.violation}
is created with
information about the contract assertion and evaluation
that triggered the violation.
Storage for \tcode{V}
is allocated in an unspecified manner
except as noted in \ref{basic.stc.dynamic.allocation}.
The lifetime of \tcode{V} persists until
contract-violation handling completes.
\item
If the contract assertion
has an assertion-control object $o$
whose type is a local-violation control type,
the expression \tcode{o.handle_contract_violation(v)} is evaluated
where \tcode{v} is a \tcode{const} lvalue referring to \tcode{V}.
If this expression has a value
equivalent to \tcode{std::contracts::violation_handled::handled}
then contract-violation handling is complete.
\item
The contract-violation handler is invoked
with a \tcode{const} lvalue referring to \tcode{v}.
\end{itemize}
...
the contract-violation handling occurs
within an active implicit handler
for that exception\iref{except.handle}.
If contract-violation handling
completes normally
...
\begin{note}
The exception can be inspected or rethrown
within the contract-violation handler.
\end{note}
...
\pnum
There is an observable checkpoint\iref{intro.abstract} $C$
that happens after contract-violation handling completes normally
such that any other evaluation
...
\pnum
If contract-violation handling
during the evaluation of a function contract assertion\iref{dcl.contract.func}
exits via an exception,
...
\begin{note}
If contract-violation handling
during the evaluation of an \grammarterm{assertion-statement}\iref{stmt.contract.assert}
exits via an exception,
