...
\pnum
%TODO: define assertion control facet

\pnum
%TODO: Define the trivial facet

\pnum
Every contract assertion $C$ has associated with it
an \defn{assertion control object}
that is a constexpr variable whose type and value
are determined as follows:

\begin{itemize}
\item If $C$ has an \grammarterm{assertion-control-specifier}
$S$ then the variable's type is deduced from
$S$ and its initializer is $S$.
\item Otherwise, the variable's type is
participates in the trivial assertion-control facet
and no other facets, and its value is unspecified.
\end{itemize}

\pnum
Every assertion control object shall participate in the
tirvial assertion-control facet.

\pnum
%TODO: define all other facets
