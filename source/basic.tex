...
\item
  ...
  to a virtual base class\ubdef{lifetime.outside.pointer.virtual} or to a base class thereof, or
...
all evaluations have an implicit precondition assertion that
they will not produce an indeterminate value
...
%
%  action: add \ubdef{basic.indet.value} at the appropriate location in the reworded paragraph
...
\item
\indextext{side effects}%
has a side effect on a memory location\iref{intro.memory} or
\item
...
then
\begin{itemize}
\item the result of the operation is an indeterminate value, and
\item if the operation has a side effect on a memory location,
an indeterminate value is stored in that memory location.
\end{itemize}
\begin{note}
...
that such an indeterminate value will
not be produced by any operation\iref{basic.contract.implicit},
and using that value can have undefined behavior.
\end{note}
\begin{note}
...
%
%  action: add \ubdef{intro.execution.unsequenced.modification} at the appropriate location in the reworded paragraph
...
\pnum
The implementation may assume\ubdef{intro.progress.stops} that any thread will eventually do one of the
