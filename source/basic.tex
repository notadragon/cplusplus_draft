...
%TODO: ubdef --- intro.object.implicit.pointer
%
%  wording: [intro.object]/11: Further, after implicitly creating objects within a specified region of storage, some operations are described as producing a pointer to a suitable created object. These operations select one of the implicitly-created objects whose address is the address of the start of the region of storage, and produce a pointer value that points to that object, if that value would result in the program having defined behavior. If no such pointer value would give the program defined behavior, the behavior of the program is undefined.
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- basic.align.object.alignment
%
%  wording: [basic.align]/1: Attempting to create an object ([intro.object]) in storage that does not meet the alignment requirements of the object's type is undefined behavior.
%
%  runtime-checkable: Yes
%
%  locally-checkable: Yes
%
%  checking: Insert alignment check
%
%  replacement: None
...
%TODO: ubdef --- lifetime.outside.pointer.member
%
%  wording: [basic.life]/7: [...] The program has undefined behavior if [...] the pointer is used to access a non-static data member or call a non-static member function of the object, [...]
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- lifetime.outside.pointer.static.cast
%
%  wording: [basic.life]/7: [...] The program has undefined behavior if [...] the pointer is used as the operand of a \tcode{static_cast} ([expr.static.cast]), except when the conversion is to pointer to $cv$ \tcode{void}, or to pointer to $cv$ \tcode{void} and subsequently to pointer to $cv$ \tcode{char}, $cv$ \tcode{unsigned char}, or $cv$ \tcode{std::byte} ([cstddef.syn]) [...]
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- lifetime.outside.pointer.virtual
%
%  wording: [basic.life]/7: [...] The program has undefined behavior if [...] the pointer is implicitly converted ([conv.ptr]) to a pointer to a virtual base class [...]
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- lifetime.outside.pointer.dynamic.cast
%
%  wording: [basic.life]/7: [...] The program has undefined behavior if [...] the pointer is used as the operand of a \tcode{dynamic_cast} ([expr.dynamic.cast]).
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- lifetime.outside.glvalue.member
%
%  wording: [basic.life]/8: [...] The program has undefined behavior if [...] the glvalue is used to call a non-static member function of the object [...]
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- lifetime.outside.glvalue.virtual
%
%  wording: [basic.life]/8: [...] The program has undefined behavior if [...] the glvalue is bound to a reference to a virtual base class ([dcl.init.ref]) [...]
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- lifetime.outside.glvalue.dynamic.cast
%
%  wording: [basic.life]/8: [...] The program has undefined behavior if [...] the glvalue is used as the operand of a \tcode{dynamic_cast} ([expr.dynamic.cast]) or as the operand of \tcode{typeid}.
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- original.type.implicit.destructor
%
%  wording: [basic.life]/11: If a program ends the lifetime of an object of type \tcode{T} with static ([basic.stc.static]), thread ([basic.stc.thread]), or automatic ([basic.stc.auto]) storage duration and if \tcode{T} has a non-trivial destructor, and another object of the original type does not occupy that same storage location when the implicit destructor call takes place, the behavior of the program is undefined.
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- creating.within.const.complete.obj
%
%  wording: [basic.life]/12: Creating a new object within the storage that a const, complete object with static, thread, or automatic storage duration occupies, or within the storage that such a const object used to occupy before its lifetime ended, results in undefined behavior.
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
%
%  checking: Track whether storage is associated with a \tcode{const} object
%
%  replacement: None
...
%TODO: ubdef --- basic.indet.value
%
%  wording: [basic.indet]/2: Except in the following cases, if an indeterminate value is produced by an evaluation, the behavior is undefined, [...]
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
%
%  checking: Track whether storage has been initialized
%
%  replacement: Only for built-in types: initialise default-initialised variables with erroneous value
...
%TODO: ubdef --- basic.stc.alloc.dealloc.constraint
%
%  wording: [basic.stc.dynamic.general]/3: If the behavior of an allocation or deallocation function does not satisfy the semantic constraints specified in [basic.stc.dynamic.allocation] and [basic.stc.dynamic.deallocation], the behavior is undefined.
%
%  runtime-checkable: Partially: some constraints can be checked locally (e.g., allocation function does not return null); others cannot be checked at all.
%
%  locally-checkable: Partially
%
%  checking: Insert checks where possible
%
%  replacement: None
...
%TODO: ubdef --- basic.stc.alloc.zero.dereference
%
%  wording: [basic.stc.dynamic.allocation]/2: The effect of indirecting through a pointer returned from a request for zero size is undefined.
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- basic.stc.alloc.dealloc.throw
%
%  wording: [basic.stc.dynamic.deallocation]/4: If a deallocation function terminates by throwing an exception, the behavior is undefined.
%
%  runtime-checkable: Make ill-formed via \cite{P3424R0}
%
%  locally-checkable: ---
%
%  checking: ---
...
%TODO: ubdef --- basic.compound.invalid.pointer
%
%  wording: [basic.compound]/4: If a pointer value $P$ is used in an evaluation $E$ and $P$ is not valid in the context of $E$, then the behavior is undefined if $E$ is an indirection ([expr.unary.op]) or an invocation of a deallocation function ([basic.stc.dynamic.deallocation]) [...]
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
...
%TODO: ubdef --- intro.execution.unsequenced.modification
%
%  wording: [conv.rank]/10: The behavior is undefined if a side effect on a memory location ([intro.memory]) or starting or ending the lifetime of an object in a memory location is unsequenced relative to another side effect on the same memory location, starting or ending the lifetime of an object occupying storage that overlaps with the memory location, or a value computation using the value of any object in the same memory location, and the two evaluations are not potentially concurrent ([intro.multithread]).
%
%  runtime-checkable: Yes
%
%  locally-checkable: Yes
%
%  checking: Identify all potential read operations that are not sequenced with respect to each given write operation; insert checks to identify if those operations are referencing the same address
%
%  replacement: Sequence operations in some unspecified order
...
%TODO: ubdef --- intro.races.data
%
%  wording: [intro.races]/17: Any such data race results in undefined behavior.
%
%  runtime-checkable: Partially
%
%  locally-checkable: No
%
%  checking: Track from which threads memory is accessed and when accesses synchronise with each other; only practical for a subset of cases (see TSan)
%
%  replacement: Make all primitive memory accesses implicitly atomic
...
%TODO: ubdef --- intro.progress.stops
%
%  wording: [intro.progress]/1: The implementation may assume that any thread will eventually do one of the following: terminate, invoke the function \tcode{std::this_thread::yield} ([thread.thread.this]), make a call to a library I/O function, perform an access through a volatile glvalue, perform an atomic or synchronization operation other than an atomic modify-write operation ([atomics.order]), or continue execution of a trivial infinite loop ([stmt.iter.general]).
%
%  runtime-checkable: No
%
%  locally-checkable: No
%
%  checking: No checking strategy exists as whether a thread will make progress is undecidable
%
...
%TODO: ubdef --- basic.start.main.exit.during.destruction
%
%  wording: [basic.start.main]/4: If \tcode{std::exit} is invoked during the destruction of an object with static or thread storage duration, the program has undefined behavior.
%
%  runtime-checkable: Yes
%
%  locally-checkable: No
%
%  checking: Track whether static or thread-local objects are currently being destroyed
%
%  replacement: None
...
%TODO: ubdef --- basic.start.term.use.after.destruction
%
...
%  locally-checkable: No
%
%  checking: Track the lifetime of static objects
%
%  replacement: None
...
%TODO: ubdef --- basic.start.term.signal.handler
%
...
%  locally-checkable: No
%
%  checking: Library precondition on Standard Library functions
%
%  replacement: None (library UB, out of scope for this paper)
...
as components of other language contracts.
...
A contract violation is always
the consequence of incorrect program code.
\end{note}
...
Explicit contract assertions are never evaluated with the assume semantic.
\begin{note}
...
would evaluate to \tcode{true}
the evaluation
has no effect;
otherwise, the evaluation has undefined behavior.

\pnum
\begin{note}
The predicate of a contract assertion
...
may have an
\defn{implicit precondition assertion} $C$ applied to it.
If so,
the evaluation of $C$
is sequenced before
...
A built-in operation $O$
may have an
\defn{implicit postcondition assertion} $C$ applied to it.
If so,
the evaluation of $C$
is sequenced after
the evaluation of $O$.
...
except the violation of a contract assertion
evaluated with the assume semantic
has an implicit precondition assertion
that it does not occur.

%NOTE: The above exception is not strictly needed, but it is
%      here to avoid the perception of an infinite recursion in
%      the specification.

\pnum
\begin{note}
The implicit precondition assertion
that an operation with undefined behavior
does not occur
will be evaluated (and thus violated)
if the operation
is reached during execution.
There are no further requirements on the behavior if
control continues after the evaluation of the
contract assertion.
\end{note}

\pnum
\begin{note}
Erroneous behvior with
an implicit precondition assertion that it does not occur
is equivalent to undefined behavior
when that assertion is evaluated with the
assume semantic\iref{basic.conttract.eval}
but in all other cases specifies behavior.
\end{note}
