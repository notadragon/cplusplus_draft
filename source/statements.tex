...
of the return value
with unspecified erroneous values,
with an implicit precondition assertion
that this does not occur\iref{basic.contract.implicit}\ubdef{stmt.return.flow.off}.
%WORDINGNOTE: \textbf{ubdef:} \hfil stmt.return.flow.off\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] a value-returning function flowing off its end without a \tcode{return}.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Equivalent to implicitly inserting a \mbox{\tcode{contract_assert(false)}} at end of the \emph{function-body}.
%
%  \item[\textbf{replacement:}] As with automatic variables, we can initialize the storage for the return value with erroneous values.
%
%  \item[\textbf{implementations:}] exhaustively caught by UBSan's \tcode{return} check, constant evaluation, and the P3850 prototype (all available semantics, both compilers).
...
%WORDINGNOTE: \textbf{ubdef:} \hfil stmt.return.coroutine.flow.off\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] a value-returning coroutine flowing off its body without a \tcode{co_return}.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert \mbox{\tcode{contract_assert(false)}} at end of \emph{function-body} if no \tcode{return_void} function is provided
%
%  \item[\textbf{replacement:}] No, we cannot fabricate an argument of a type suitable to invoke one of possibly many overloads of \tcode{return_value} on the promise type.
%
%  \item[\textbf{implementations:}] Caught by the P3850 prototype.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil stmt.dcl.local.static.init.recursive\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] recursively re-entering a block-scope \tcode{static} variable's initializer.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Insert a recursion counter into a guard for static and thread-local object construction
%
