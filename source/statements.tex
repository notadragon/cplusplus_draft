...
\end{codeblock}
\end{example}
%TODO: ubdef --- stmt.return.flow.off
%  wording: [stmt.return]/4: Otherwise, flowing off the end of a function that is neither \tcode{main} ([basic.start.main]) nor a coroutine ([dcl.fct.def.coroutine]) results in undefined behavior.
...
\end{itemize}
%TODO: ubdef --- stmt.return.coroutine.flow.off
%  wording: [stmt.return.coroutine]/3: If a search for the name \tcode{return_void} in the scope of the promise type finds any declarations, flowing off the end of a coroutine's \emph{function-body} is equivalent to a \mbox{\tcode{co_return}} with no operand; otherwise flowing off the end of a coroutine's \emph{function-body} results in undefined behavior.
%  runtime-checkable: Yes
%  locally-checkable: Yes
...
\end{codeblock}
\end{example}
%TODO: ubdef --- stmt.dcl.local.static.init.recursive
%  wording: [stmt.dcl]/3: If control re-enters the declaration recursively while the variable is being initialized, the behavior is undefined.
%  runtime-checkable: Yes
