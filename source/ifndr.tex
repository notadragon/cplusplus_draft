...
int main() {
    int __x;    // IFNDR, \tcode{__x} is reserved because it starts with \tcode{__}
    int _Y;     // IFNDR, \tcode{_Y} is reserved because it starts with \tcode{_} followed by a capital letter
    int x__y;   // IFNDR, \tcode{x__y} is reserved because it contains \tcode{__}
}
\end{codeblock}
\end{example}
...
\item \tcode{D} is an inline or templated function or variable, and
...
template <typename T> concept C = requires {
  new decltype((void)T{});  // IFNDR, invalid use of \tcode{void} in all instantiations
};
\end{codeblock}
\end{example}

\ifndrdescription{expr.const.const.immediate.var.def}
...
must be odr-used
only from
immediate function contexts.
No diagnostic is required
if the odr-use cannot reach
the defining declaration
of the variable
or the defining declaration
cannot reach
the odr-use.
...
#include "a.h"
struct X;
static_assert(!is_complete<X>::value);  // IFNDR, different specialization selected in \tcode{"b.cpp"}
\end{codeblocktu}
\begin{codeblocktu}{Source file \tcode{"b.cpp"}}
#include "a.h"
struct X { };
static_assert(is_complete<X>::value);   // IFNDR, different specialization selected in \tcode{"a.cpp"}
\end{codeblocktu}
\end{example}
