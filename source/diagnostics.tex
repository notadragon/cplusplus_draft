...
documenting \Cpp{} program assertions,
a mechanism for disabling the assertion checks
through defining the macro \tcode{NDEBUG},
and
a mechanism to integrate with the contract-violation handler
through defining the macro \mname{STDC_WANT_ASSERT_USES_CONTRACTS}.
...
at the point in the source file where
\tcode{<cassert>} or \tcode{<assert.h>}
is included,
...
the evaluation of \mname{VA_ARGS},
it propagates normally.
\end{note}
\item
If the evaluation yields \tcode{true} there are no further effects.
\item
Otherwise, if \mname{STDC_WANT_ASSERT_USES_CONTRACTS}
is defined as a macro name
at the point in the source file where
\tcode{<cassert>} or \tcode{<assert.h>}
is included,
the \tcode{assert} macro's expression
\begin{itemize}
\item
invokes the contract-violation handler\iref{basic.contract.handler}, and
\item
then, if the contract-violation handler
returns normally
or
exits via an exception,
calls \tcode{abort()}.
\end{itemize}
The \tcode{contract_violation} object
...
\item
  The \tcode{kind}
  will be
  the value \tcode{cassert}.
\item
  The \tcode{detection_mode}
  will be
  the value \tcode{predicate_false}.
\item
  The \tcode{evaluation_semantic}
  will be
  the value \tcode{enforce}.
\item
  The \tcode{location} will,
  if populated,
  represent the source file, line number,
  and name of the enclosing function.
\item
  The \tcode{comment} will,
  if populated,
  contain \tcode{\#}\mname{VA_ARGS}.
\end{itemize}
\item
