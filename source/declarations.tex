...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.type.cv.modify.const.obj\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] modifying a \tcode{const} object, even via a \tcode{const_cast}-obtained pointer.
%
...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.type.cv.access.volatile\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] accessing a \tcode{volatile}-qualified object through a non-\tcode{volatile} glvalue.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether storage is associated with a \tcode{volatile} object
%
%  \item[\textbf{replacement:}] None
%
...
\nontermdef{simple-type-specifier}\br
    ...
    \opt{nested-name-specifier} simple-template-name\br
    pack-index-template-name\br
    \keyword{char}\br
    \keyword{char8_t}\br
    \keyword{char16_t}\br
    \keyword{char32_t}\br
...
\grammarterm{simple-template-name}, and/or
...
a deduced class type\iref{dcl.type.class.deduct} if
\begin{itemize}
\item
it is of the form
\opt{\keyword{typename}} \opt{\grammarterm{nested-name-specifier}} \grammarterm{simple-template-name}, or
\item
it is of the form
...
if any, shall be non-dependent and the
\grammarterm{simple-template-name},
\grammarterm{pack-index-template-name}, or
...
\pnum
The \grammarterm{initializer} of an \grammarterm{init-declarator}
shall not be present unless
the declarator declares a variable and
the host scope\iref{basic.scope.scope} of the declaration
is the same as its target scope.

\pnum
The optional \grammarterm{requires-clause} of an
\grammarterm{init-declarator} or \grammarterm{member-declarator}
shall not be present unlesss the declarator declares a
...
of an \grammarterm{init-declarator}
shall not be present unless
...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.ref.incompatible.function\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] binding a reference to a function of a call-incompatible type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track the types of all functions based on their addresses
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] UBSan's \tcode{function} (part of \tcode{undefined})
%  check catches it on Clang.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.ref.incompatible.type\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] binding a reference to an object via a type-incompatible (strict-aliasing) access.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.ref.uninitialized.reference\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] evaluating a reference before it has actually been initialized.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether references have been initialised
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
A parameter list consisting of a single unnamed non-object parameter
whose type is the same as\iref{temp.over.link} \keyword{void}
is interpreted as an empty parameter list.
...
S<int> x;                       // error: \#1 has a parameter of type \tcode{void}
...
void f() pre (b1) pre ([]{ return b2; }()); // ill-formed, no diagnostic required: closures have
                                            // different types
...
B v1 = B{.a1=1, .b=2};          // the explicitly initialized elements are \textnormal{[}\tcode{A}, \tcode{B::b}\textnormal{]}
B v2 = B{.a1=1, .a2=2, .b=3};   // the explicitly initialized elements are \textnormal{[}\tcode{A}, \tcode{B::b}\textnormal{]}
B v3 = B{A{1, 2}, .b=3};        // the explicitly initialized elements are \textnormal{[}\tcode{A}, \tcode{B::b}\textnormal{]}
B v4 = B{A{}, .a2=1, .b=3};     // error: \tcode{A} initialized two different ways
C v5 = C{.a1=4};                // the explicitly initialized elements are \textnormal{[}\tcode{C::a1}\textnormal{]}
\end{codeblock}
\end{example}
...
The associated elements of the non-static data members
...
E i{.na=1, .e=2};                   // error: the lookup set for \tcode{na} finds \tcode{NonAggr},
                                    // which is not an aggregate base of \tcode{E}
\end{codeblock}
\end{example}
...
If the \grammarterm{initializer-clause}{s} of
the \grammarterm{initializer-list}
are interpreted as arguments of a constructor call\iref{over.match.list},
any default arguments used are evaluated in the order as if
an \grammarterm{initializer-clause} were present for the corresponding parameter.
\begin{note}
...
int main() {
  S(1, 2); std::cout << '\n';   // unspecified order; prints \tcode{12} or \tcode{21}
  S{1, 2}; std::cout << '\n';   // prints \tcode{12}
  S{1};                         // prints \tcode{12}
}
\end{codeblock}
...
\begin{itemize}
\item be a special member function\iref{special},
comparison operator function\iref{over.binary, class.compare.default}, or
postfix increment or decrement operator function\iref{over.inc.default},
and
...
\item
  ...
  of type ``lvalue reference to \tcode{const C}'',
...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.fct.def.coroutine.resume.not.suspended\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] calling \tcode{resume()} on a coroutine handle that is not suspended.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No, requires additional instrumentation
%
%  \item[\textbf{checking:}] Track (in a thread-safe fashion)
%  the suspension state associated with every coroutine handle
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] AddressSanitizer will catch many cases
%  as a use-after-free.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.fct.def.coroutine.destroy.not.suspended\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] calling \tcode{destroy()} on a coroutine handle that is not suspended.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No, requires additional instrumentation
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] AddressSanitizer (use-after-free/double-free) catches it on both compilers.
%  \end{ubsummaryitemize}
...
There is an implicit precondition assertion
that the converted expression
would evaluate to \tcode{true}\iref{basic.contract.implicit}\ubdef{dcl.attr.assume.false}.
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.attr.assume.false\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] reaching an \tcode{[[assume(expr)]]} attribute while \tcode{expr} is false.
%
%  \item[\textbf{runtime-checkable:}] Not in general, UB is based on the hypothetical result of
%  an arbitrary expression that cannot be evaluated if it might have side effects.
%
%  \item[\textbf{locally-checkable:}] Yes, if checkable at all.
%
%  \item[\textbf{checking:}] No automatic checking strategy is possible because the predicate cannot be, in general, proven to be free of side effects; instead, the user has to change \tcode{[[assume(x)]]} to \mbox{\tcode{contract_assert<may_be_assumed>(x)}} and select an appropriate evaluation semantic
%
%  \item[\textbf{replacement:}] Ignore the assumption
%
%  \item[\textbf{implementations:}] P3850-native check covers the side-effect-free \tcode{.pure} subgroup with every semantic on both compilers, plus a \tcode{constexpr} catch on both compilers; the opaque \tcode{.nonpure} subgroup supports only \emph{assume} and \emph{ignore}.
%  \end{ubsummaryitemize}
...
int divide_by_32(int x) {
  [[assume(x >= 0)]];       // Implicit precondition that \tcode{x >= 0}
  return x/32;              // When the precondition is assumed, the instructions produced
                            // for the division may omit handling of negative values.
}
int f(int y) {
  [[assume(++y == 43)]];    // \tcode{y} is not incremented
  return y;                 // When the precondition is assumed, can return \tcode{42}
}
\end{codeblock}
\end{example}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil dcl.attr.noreturn.eventually.returns\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] a \tcode{[[noreturn]]}-declared function returns to its caller anyway.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert \tcode{post(false)}
%
%  \item[\textbf{replacement:}] Return normally.  This would require both the caller and callee translation
%  unit to ignore instead of assume that the function does not return.
%
%  \item[\textbf{implementations:}] UBSan's \tcode{unreachable} check (own diagnostic) catches it on both Clang and GCC.
%  \end{ubsummaryitemize}
\begin{note}
The function can
