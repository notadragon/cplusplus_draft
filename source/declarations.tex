...
\nontermdef{postcondition-specifier}\br
  \keyword{post} \opt{attribute-specifier-seq} \opt{postcondition-captures-clause}
  \bnfindent \terminal{(} \opt{result-name-introducer} conditional-expression \terminal{)}
\end{bnf}
...
\pnum
The \defnadj{constituent}{statements} of a contract assertion
are, in order:
\begin{itemize}
\item
For every postcondition capture
associated with the contract assertion, if any,
a variable declaration
whose name, type, and initializer
are the same as the
postcondition capture.
\item
If there is a
\grammarterm{result-name-introducer},
a variable declaration
whose name and type
are the same as
the result binding.
\item
An \keyword{if} statement
whose condition
is the predicate of
...
\pnum
A \grammarterm{function-contract-specifier-seq} $F_1$
is the same as
a \grammarterm{function-contract-specifier-seq} $F_2$
if $F_1$ and $F_2$ consist of
...
\begin{itemize}
\item
their constituent statements $S_1$ and $S_2$ would
...
that are contained within $S_1$ and $S_2$,
...
\rSec2[dcl.contract.capture]{Postcondition Captures}

\begin{bnf}
\nontermdef{postcondition-captures-clause}\br
  \terminal{[} postcondition-capture-list \terminal{]}
\end{bnf}

\begin{bnf}
\nontermdef{postcondition-capture-list}\br
  postcondition-capture\br
  postcondition-capture-list \terminal{,} postcondition-capture
\end{bnf}

\begin{bnf}
\nontermdef{postcondition-capture}\br
  postcondition-parameter-capture\br
  postcondition-init-capture
\end{bnf}

\begin{bnf}
\nontermdef{postcondition-parameter-capture}\br
  identifier \opt{\terminal{...}}
\end{bnf}

\begin{bnf}
\nontermdef{postcondition-init-capture}\br
  \opt{\terminal{...}} identifier initializer
\end{bnf}

\pnum
A \defnadj{postcondition}{capture}
is associated with a postcondition assertion
and declares a variable
that inhabits the contract-assertion scope\iref{basic.scope.contract}
of its associated postcondition assertion.

A \grammarterm{postcondition-capture}
containing an ellipsis
is a pack expansion that declares
a postcondition-capture pack\iref{temp.variadic}.

The \grammarterm{identifier}
of a \grammarterm{postcondition-parameter-capture}
shall denote a parameter
of the function
that the postcondition assertion is associated with.
If the \grammarterm{postcondition-parameter-capture}
contains an ellipsis
the parameter
shall be a parameter pack.
The initializer of the
introduced postcondition capture
is the identifier.
\begin{note}
The variable introduced by a \grammarterm{postcondition-parameter-capture}
inhabits the contract-assertion scope
of the postcondition assertion
and thus the \grammarterm{identifier}
in the initializer
will refer to the function parameter
and not the capture.
\end{note}

Each postcondition capture
introduces a variable
with automatic storage duration
whose placeholder type is \keyword{auto}
and
whose type is deduced from its initializer.
\begin{note}
In the initializer,
an expression naming a variable,
including a function parameter,
will have \keyword{const} added to its type\iref{expr.prim.id.qual,expr.prim.id.unqual}.
\end{note}
%WORDINGNOTE: Here is a place we will need to modify when we make a final decision
% about whether the type of the variable should be const and whether it should
% be `const`-ified in the predicate.
