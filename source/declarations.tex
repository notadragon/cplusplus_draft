...
\nontermdef{precondition-specifier}\br
  \keyword{pre} \opt{assertion-control-specifier} \opt{attribute-specifier-seq} \terminal{(} conditional-expression \terminal{)}
\end{bnf}

\begin{bnf}
\nontermdef{postcondition-specifier}\br
  \keyword{post} \opt{assertion-control-specifier} \opt{attribute-specifier-seq} \terminal{(} \opt{result-name-introducer} conditional-expression \terminal{)}
\end{bnf}
...
\item
both $C_1$ and $C_2$
have \grammarterm{assertion-control-specifiers}
or neither do,
\item
if present,
their assertion-control specifiers $O_1$ and $O_2$
would satisfy the one-definition rule\iref{basic.def.odr}
if placed as the arguments
of contract-control expressions\iref{expr.contract.control}
in function definitions on
the declarations $D_1$ and $D_2$, respectively,
except for
\begin{itemize}
\item
renaming of the parameters of \placeholder{f}, and
\item
renaming of template parameters of
a template enclosing \placeholder{f}
\end{itemize}
if $D_1$ and $D_2$ are in different translation units,
corresponding entities defined within each predicate
behave as if there is a single entity with a single definition, 
and,
\item
...
\nontermdef{using-directive}\br
    \opt{attribute-specifier-seq} \keyword{using} \opt{\keyword{contract_control}} \keyword{namespace} \opt{nested-name-specifier} namespace-name \terminal{;}\br
    \opt{attribute-specifier-seq} \keyword{using} \opt{\keyword{contract_control}} \keyword{namespace} splice-specifier \terminal{;}
\end{bnf}
...
\pnum
The optional \keyword{contract_control} keyword, if present,
indicates a using directive that will be considered
only within assertion-control scopes\iref{basic.scope.contract.control,basic.lookup}.
