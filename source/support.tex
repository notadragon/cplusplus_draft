...
#define @\defnlibxname{cpp_lib_assert_can_use_contracts}@          yyyymmL // freestanding, also in \libheader{assert.h} and \libheader{cassert}
...
#define @\defnlibxname{cpp_lib_contracts_api}@                     yyyymmL // freestanding, also in \libheader{contracts}
...
\tcode{manual}  & A manually triggered violation                   \\ \rowsep
\tcode{cassert} & The \tcode{assert} macro\iref{assertions.assert} \\ \rowsep
\end{floattable}
...
\topline
\lhdr{Name}                  & \rhdr{Meaning}                    \\ \capsep
\tcode{unspecified}          & The mode of detection was not provided to the contract-violation handling process \\ \rowsep
...
\rSec2[support.contract.handle]{Manual violation-handler invocation}
...
\item
  ...
  with a \tcode{contract_violation} object
  ...
  \item
    ...
    if populated,
    will be
    the \tcode{comment} passed to this function.
  \item
    The \tcode{location},
    if populated,
    will be
    the \tcode{location} passed to this function or
    the location of the function invocation.
  \item
    The \tcode{kind}
    will be
    \tcode{manual}.
  \item
    The \tcode{detection_mode}
    will be
    \tcode{unspecified}.
  \item
    The \tcode{evaluation_semantic}
    will be
    \tcode{enforce}.
  \end{itemize}
\item
  If the violation handler returns normally,
  the program is contract-terminated\iref{basic.contract.eval}.
\end{itemize}

\pnum
\throws
Nothing if the \tcode{nothrow_t} overload is selected,
otherwise
any exception thrown
from the contract-violation handler.
...
\pnum
\effects
Contract-terminate the program\iref{basic.contract.eval}.
\end{itemdescr}
