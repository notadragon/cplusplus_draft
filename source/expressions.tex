...
\nontermdef{unary-expression}\br
    ...
    assertion-control-expression\br
...
\rSec3[expr.contract.control]{Contract-control expression}

\begin{bnf}
\nontermdef{assertion-control-expression}\br
    \terminal{contract_control} \terminal{(} constant-expression \terminal{)}
\end{bnf}

\pnum
An \grammarterm{assertion-control-expression}
yields the same value and has the same value category
as its \grammarterm{constant-expression}.

\begin{note}
The \grammarterm{constant-expression} is
manifestly constant-evaluated.
Note that the \grammarterm{assertion-control-expression}
introduces a new assertion-control scope\iref{basic.scope.contract.control}
in which name lookup will search
namespaces referred to by \grammarterm{using-directives}
that have the optional \keyword{contract_control} keyword\iref{namespace.udir,basic.lookup}.
\end{note}
