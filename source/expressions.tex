...
\end{note}
If the mathematical result of evaluating an arithmetic expression is neither
\begin{itemize}
\item
...
\end{itemize}
then the result is an unspecified erroneous value
with an implicit precondition assertion
that it does not occur\iref{basic.contract.implicit}\ubdef{expr.expr.eval}.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.expr.eval\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] signed integer \tcode{+}, \tcode{-}, \tcode{\char42}, and unary negation overflow (a subset of the general expression-evaluation rule).
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert a check of whether the given arguments will produce a valid value
%
%  \item[\textbf{replacement:}] Coerce into erroneous value, either explicitly or by using the result of the underlying instruction as-is
%
%  \item[\textbf{implementations:}] exhaustively caught by UBSan's \tcode{signed-integer-overflow}, constant evaluation, and the P3850 prototype (GCC nonthrowing-only, Clang all semantics).
%  \end{ubsummaryitemize}
\begin{note}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.basic.lvalue.strict.aliasing.violation\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] accessing an object through a glvalue of a dissimilar type (strict-aliasing violation).
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track the dynamic type of the object in storage and verify the access glvalue's type is similar to it
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
%  \end{ubsummaryitemize}
\begin{footnote}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.basic.lvalue.union.initialization\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] defaulted union copy/move construction whose argument isn't a live similar-type object.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether storage is associated with an object of correct type within its lifetime
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] rejected during constant evaluation on both GCC and Clang.
%  \end{ubsummaryitemize}
\begin{note}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.type.reference.lifetime\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] evaluating (naming) a reference to an object whose lifetime has ended.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track object and storage lifetimes
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
are not valid for the object's type,
the prvalue result is an unspecified erroneous value,
and there is an implicit precondition
that this does not occur\iref{basic.contract.implicit}\ubdef{conv.lval.valid.representation}.
%WORDINGNOTE: \textbf{ubdef:} \hfil conv.lval.valid.representation\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] lvalue-to-rvalue conversion of an invalid bit pattern for
%  an object of non-class type
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert a check of whether the value is a valid value representation for the object's type
%
%  \item[\textbf{replacement:}] Coerce invalid value representations into erroneous values
%
%  \item[\textbf{implementations:}] the \tcode{bool} and \tcode{enum} subgroups are
%  caught natively by the P3850 prototype.  With GCC potentially-throwing semantics
%  are not supported while with Clang they are supported.
%  UBSan's \tcode{bool}/\tcode{enum} checks (both in the
%  \tcode{undefined} group) check both categories.  The \tcode{int} and \tcode{float}
%  subgroups have no invalid representation on commonly available platforms.
%  \end{ubsummaryitemize}
\begin{example}
\begin{codeblock}
bool f() {
...
Otherwise, the result is an unspecified erroneous value
with an implicit precondition that this does not occur\ubdef{conv.double.out.of.range}.
%WORDINGNOTE: \textbf{ubdef:} \hfil conv.double.out.of.range\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] converting a floating-point value to a type that cannot represent it.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert a check of whether the value is one that can be converted
%
%  \item[\textbf{replacement:}] Coerce into erroneous value
%
%  \item[\textbf{implementations:}] None
%  \end{ubsummaryitemize}
...
An unspecified erroneous value is produced
if the truncated value cannot be represented
in the destination type,
with an implicit precondition that this does not occur\iref{basic.contract.implicit}\ubdef{conv.fpint.float.not.represented}.
%WORDINGNOTE: \textbf{ubdef:} \hfil conv.fpint.float.not.represented\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] float-to-integer conversion whose truncated value isn't
...
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert a check of whether the value is valid
%
%  \item[\textbf{replacement:}] Coerce into erroneous value
%
%  \item[\textbf{implementations:}] Checked by UBSan's \tcode{float-cast-overflow},
%  in constant evaluation, and implemented natively by the P3850 prototype.
%  \end{ubsummaryitemize}
\begin{note}
...
an unspecified erroneous value is produced
with an implicit precondition that this does not occur\ubdef{conv.fpint.int.not.represented}.
%WORDINGNOTE: \textbf{ubdef:} \hfil conv.fpint.int.not.represented\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] integer/unscoped-enum to floating-point conversion whose value
%  isn't representable.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
...
%WORDINGNOTE: \textbf{ubdef:} \hfil conv.ptr.virtual.base\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] converting a derived-class pointer to a virtual base when the
%  source isn't a valid, in-lifetime object.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Only for the null pointer case
%
%  \item[\textbf{checking:}] Track whether storage is associated with an object of correct type within its lifetime or an object currently being constructed or destroyed; insert null pointer check
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] Many cases caught by UBSan's \tcode{vptr} check on both
%  compilers.
%  \end{ubsummaryitemize}
Otherwise,
...
%WORDINGNOTE: \textbf{ubdef:} \hfil conv.member.missing.member\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] converting a base pointer-to-member to a derived one that could
%  not actually contain that member.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track which type the pointer to member originated from.
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] GCC's \tcode{object-size} check flags some cases
%  (heuristic; stock and integrated); constant evaluation exhaustive on both compilers.
%  \end{ubsummaryitemize}
Otherwise,
...
\pnum
The non-static data members in the closure type are declared in an unspecified order,
except that the members introduced for explicit captures are declared in an order
consistent with that of those captures.
...
The non-static data members corresponding to the \grammarterm{init-capture}{s}
and the (reference) variables declared by \grammarterm{init-capture}{s}
without corresponding non-static data members
are initialized as indicated by the corresponding \grammarterm{initializer}
...
when the \grammarterm{lambda-expression} is evaluated,
...
\nontermdef{compound-requirement}\br
    \terminal{\{} expression \terminal{\}} \opt{noexcept-specifier} \opt{return-type-requirement} \terminal{;}
\end{bnf}
...
\item
If the \grammarterm{noexcept-specifier}\iref{except.spec} is present, then:
\begin{itemize}
\item
Substitution of template arguments (if any)
into its associated \grammarterm{constant-expression} is performed.
\item
If the value of the converted \grammarterm{constant-expression} is \tcode{true},
...
\pnum
The precondition assertions of a function call are,
in order:
\begin{itemize}
\item
if the \grammarterm{postfix-expression}
is a (possibly implicit) class member access expression
and the call
is a virtual function call,
the precondition assertions\iref{dcl.contract.func}
of the statically chosen function;
\item
the precondition assertions
of the function actually called.
\end{itemize}
The postcondition assertions of a function call are,
in order:
\begin{itemize}
\item
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.call.different.type\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] calling a function through a pointer/reference type incompatible with its actual definition.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track type information of function based on address
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by Clang's UBSan \tcode{function}/\tcode{undefined} check; rejected during constant evaluation on both GCC and Clang.
%  \end{ubsummaryitemize}
\begin{note}
...
and each precondition assertion of the function call
...
each \grammarterm{initializer-clause} in the \grammarterm{expression-list}
...
sequenced before
the evaluation of the precondition assertions of the function call,
...
all postcondition assertions of the function call
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.ref.member.not.similar\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] accessing a member through a glvalue not similar to the object's actual type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by both compilers' constant evaluators.
%  \end{ubsummaryitemize}
\begin{example}
\begin{codeblock}
struct A { int i; };
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.dynamic.cast.pointer.lifetime\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{dynamic_cast} on a non-null pointer to the wrong type or outside any object's lifetime.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Only for the null pointer case
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.dynamic.cast.glvalue.lifetime\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{dynamic_cast<T\&>} on an object of the wrong type or outside its lifetime.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether storage is associated with an object of correct type within its lifetime or an object currently being constructed or destroyed
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] rejected during constant evaluation on Clang.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.static.cast.base.class\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{static_cast}-ing a base glvalue to a reference of the wrong sibling type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] polymorphic subset caught by UBSan's \tcode{vptr} check (Clang, integrated, both \tcode{noexcept}-enforce and \tcode{noexcept}-observe) and constant evaluation (Clang); nonpolymorphic subset caught by constant evaluation (Clang).
%  \end{ubsummaryitemize}
\begin{example}
\begin{codeblock}
struct B { };
...
otherwise, an unspecified erroneous value is produced with
an implicit precondition that this does not occur\iref{basic.contract.implicit}\ubdef{expr.static.cast.enum.outside.range}.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.static.cast.enum.outside.range\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{static_cast}-ing an out-of-range integer into an unscoped enum without a fixed underlying type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert a check of whether the value is in the valid range
%
%  \item[\textbf{replacement:}] Coerce into erroneous value
%
%  \item[\textbf{implementations:}] caught by Clang via constant evaluation and UBSan's \tcode{enum} check (\tcode{-fsanitize=enum}) on the load; and, in the P3850 prototype, by a front-end check at the cast site on both compilers (all available semantics) -- closing GCC's coverage gap.
%  \end{ubsummaryitemize}
...
Otherwise, an unspecified erroneous value is produced
with an implicit precondition that this does not occur\iref{basic.contract.implicit}\ubdef{expr.static.cast.fp.outside.range}.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.static.cast.fp.outside.range\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] an explicit \tcode{static_cast} of a floating-point value outside the target type's representable range.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.static.cast.downcast.wrong.derived.type\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{static_cast}-ing a base pointer to a pointer of the wrong sibling type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Only for the null pointer case
%
%  \item[\textbf{checking:}] Track whether storage is associated with an object of correct type within its lifetime or an object currently being constructed or destroyed; insert a null pointer check
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] polymorphic subset caught by UBSan's \tcode{vptr} check (both compilers, integrated, both \tcode{noexcept}-enforce and \tcode{noexcept}-observe) and constant evaluation (Clang); nonpolymorphic subset caught by constant evaluation (Clang).
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.static.cast.does.not.contain.original.member\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{static_cast}-ing a pointer-to-member to a base lacking the original member.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by Clang's constant evaluator.
%  \end{ubsummaryitemize}
\begin{note}
...
the behavior is undefined\ubdef{expr.reinterpret.cast.invalid.pointer.value}.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.reinterpret.cast.invalid.pointer.value\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] forming a pointer via \tcode{reinterpret_cast} with no real provenance.
%
%  \item[\textbf{runtime-checkable:}] No, requires predicting whether any potential value will encounter undefined behavior
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Would require properly identifying when angelic provenance does not apply
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.unary.dereference\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] dereferencing a pointer that is null or otherwise does not point to a live object or function.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Only for the null pointer case
%
%  \item[\textbf{checking:}] Track whether storage is associated with an object of correct type within its lifetime; track whether the address is associated with a function; insert a null pointer check
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] \tcode{.nullptr} caught by a native P3850 check (all available semantics on Clang; four non-throwing semantics on GCC) and constant evaluation, both compilers; \tcode{.invalid} caught heuristically by AddressSanitizer and by constant evaluation, both compilers.
%  \end{ubsummaryitemize}
\begin{note}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.new.non.allocating.null\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] a user-replaced non-allocating placement-\tcode{new} returning null.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert \tcode{post(r: r)}
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by UBSan's \tcode{null} check (part of the \tcode{undefined} group), which flags the null pointer returned by the non-allocating placement \tcode{new}, and by both compilers' constant evaluators.
%  \end{ubsummaryitemize}
Otherwise,
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.delete.mismatch\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] using \tcode{delete[]} on a pointer from single-object \tcode{new}.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track pointer provenance, verify the delete form matches the allocation form (single-object versus array)
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] exhaustively caught by ASan's \tcode{alloc_dealloc_mismatch} on both compilers; rejected during constant evaluation on both GCC and Clang.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.delete.array.mismatch\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] applying single-object \tcode{delete} to a pointer from array \tcode{new[]}.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.delete.dynamic.type.differ\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{delete} through a pointer whose static type differs from its dynamic type, with no virtual destructor.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track dynamic type of non-polymorphic objects
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught by ASan's \tcode{new-delete-type-mismatch} on both compilers via allocation-size mismatch; rejected during constant evaluation on Clang.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.delete.dynamic.array.dynamic.type.differ\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] \tcode{delete[]} whose static type differs from the array's dynamic type.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
...
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] Partially caught by ASan's \tcode{new-delete-type-mismatch} on both compilers when element sizes differ; rejected during constant evaluation on both GCC and Clang.
%  \end{ubsummaryitemize}
...
the expression \tcode{\&\grammarterm{id-expression}} shall be well-formed
when considered as an unevaluated operand,
except that the function $F$ selected as described in \ref{over.over}
may be deleted\iref{dcl.fct.def.delete};
$R$ represents $F$.
\item
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.mptr.oper.not.contain.member\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] accessing a pointer-to-member whose target object lacks that member.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track which type the pointer to member originated from and the dynamic type of non-polymorphic objects
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught heuristically by AddressSanitizer (both compilers) and by both compilers' constant evaluators.
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.mptr.oper.member.func.null\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] calling through a null pointer-to-member-function.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert null pointer check
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] Caught by both compilers' constant evaluators.
...
\tcode{/} or \tcode{\%}
is zero,
then the operator yields an unspecified erroneous value
and there is an implicit precondition that
this does not occur\ubdef{expr.mul.div.by.zero}.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.mul.div.by.zero\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] integer division/remainder and floating-point division by zero.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert a check of whether the second operand is zero
%
%  \item[\textbf{replacement:}] Produce an erroneous result after checking the second operand
%
%  \item[\textbf{implementations:}] the integer cases are caught by UBSan's \tcode{integer-divide-by-zero} check, constant evaluation, and the P3850 prototype (both compilers, all semantics); floating-point division by zero is caught by UBSan's \tcode{float-divide-by-zero} check, which is not part of the \tcode{undefined} group, and constant evaluation.
%  \end{ubsummaryitemize}
...
\tcode{(a/b)*b + a\%b} is equal to \tcode{a}; otherwise,
the operator yields an unspecified erroneous value
and there is an implicit precondition that
this does not occur\iref{basic.contract.implicit}\ubdef{expr.mul.representable.type.result}.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.mul.representable.type.result\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] integer division or remainder whose quotient overflows, e.g., \tcode{INT_MIN / -1} or the paired \tcode{INT_MIN \% -1}.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
...
%  \item[\textbf{replacement:}] Coerce into erroneous value
%
%  \item[\textbf{implementations:}] exhaustively caught by UBSan's \tcode{signed-integer-overflow} check, constant evaluation, and the P3850 prototype (both compilers, all semantics).
%  \end{ubsummaryitemize}
...
to similar\iref{conv.qual} complete object types; or
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.add.out.of.bounds\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] forming a pointer beyond an array's one-past-the-end bound.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Only if the array bound is statically known
%
%  \item[\textbf{checking:}] Track pointer provenance, insert bounds check
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] \tcode{.known} fully checked by the P3850 bounds check (all semantics, both compilers) and rejected during constant evaluation on both compilers; \tcode{.unknown} only heuristically by AddressSanitizer, and also rejected during constant evaluation on both compilers; \tcode{.pointer.arithmetic} by UBSan's \tcode{pointer-overflow} check on both compilers.
%  \end{ubsummaryitemize}
\end{itemize}
\begin{note}
...
the result is an unspecified erroneous value
and there is an implicit precondition
that this does not occur\ubdef{expr.sub.pointers.representable}.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.sub.pointers.representable\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] subtracting two pointers into the same array when the index difference overflows \tcode{std::ptrdiff_t}.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Verify that overflow does not occur when converting to \tcode{std::ptrdiff_t}
%
%  \item[\textbf{replacement:}] Coerce into erroneous value
%
%  \item[\textbf{implementations:}] None
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.add.sub.diff.pointers\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] subtracting pointers into two different, unrelated arrays.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track pointer provenance, verify both operands point into the same array object
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] caught at run time by AddressSanitizer's \tcode{pointer-subtract} check (stock and integrated) on both compilers, and rejected during constant evaluation on both GCC and Clang.
%  \end{ubsummaryitemize}
\end{itemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.add.not.similar\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] pointer arithmetic advancing by the wrong per-element size (dissimilar pointer/array types).
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] No
%
%  \item[\textbf{checking:}] Track whether storage is associated with an object of correct type
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] Rejected during constant evaluation on both compilers.
%  \end{ubsummaryitemize}
\begin{example}
\begin{codeblock}
int arr[5] = {1, 2, 3, 4, 5};
...
There is an implicit precondition that
the right operand is nonnegative and
less than the width of the promoted left operand\iref{basic.contract.implicit}\ubdef{expr.shift.neg.and.width};
if not, an unspecified erroneous value is produced.
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.shift.neg.and.width\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] shifting by a negative amount or by the type's bit width or more.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Insert check whether right operand is valid
%
%  \item[\textbf{replacement:}] Coerce into erroneous value
%
%  \item[\textbf{implementations:}] exhaustively caught by UBSan's \tcode{shift} check, constant evaluation, and the P3850 prototype (both compilers, all semantics).
%  \end{ubsummaryitemize}
...
%WORDINGNOTE: \textbf{ubdef:} \hfil expr.assign.overlap\hfil
%  \begin{ubsummaryitemize}
%  \item[\textbf{summary:}] an assignment whose source and destination overlap in storage.
%
%  \item[\textbf{runtime-checkable:}] Yes
%
%  \item[\textbf{locally-checkable:}] Yes
%
%  \item[\textbf{checking:}] Check overlap of the two address ranges
%
%  \item[\textbf{replacement:}] None
%
%  \item[\textbf{implementations:}] None
...
\end{itemize}
is reachable from the other\ifndrdef{expr.const.const.immediate.var.def}.
...
void f() {
  ...
  A aa = {1, 2};
  static A sa = {3, 4};
  // The objects \tcode{sx}, \tcode{ax}, and \tcode{aa.m}, \tcode{sa.m}, and the temporaries to
  // which \tcode{aa.r} and \tcode{sa.r} are bound, are constexpr-referenceable.
  auto lambda = [] {
    int ay;
