...
\item otherwise the behavior is undefined\ubdef{conv.lval.valid.representation}.
\end{itemize}
\begin{example}
\begin{codeblock}
bool f() {
...
with an implicit precondition that this does not occur.\ubdef{conv.double.out.of.range}.
...
An erronneous value is produced
if the truncated value cannot be represented\ubdef{conv.fpint.float.not.represented}
...
\ubdef{expr.static.cast.enum.outside.range}.
...
Otherwise, the an erroneous value is produced
with an implicit precondition that this does not occur\iref{basic.contract.impliciti}\ubdef{expr.static.cast.fp.outside.range}.
...
the second operand of \tcode{/} or \tcode{\%} is notzero\ubdef{expr.mul.div.by.zero},
...
if not, an erroneous value is produced\ubdef{expr.shift.neg.and.width}.
