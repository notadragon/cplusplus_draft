...
\end{note}
The behavior of evaluating an arithmetic expression is undefined\ubdef{expr.expr.eval}
...
in
\begin{itemize}
\item the predicate of a precondition assertion, or
\item the initializer of a postcondition capture
\end{itemize}
of a constructor of \tcode{X},
or the predicate of a postcondition assertion of a destructor of \tcode{X},
...
appears in
\begin{itemize}
\item the predicate, or
\item the initializer of a postcondition capture
\end{itemize}
of a contract assertion $C$\iref{basic.contract}
and the entity is
\begin{itemize}
\item
a variable
...
%WORDINGNOTE: If we choose to apply \tcode{const}-ification to postcondition
% captures we would add that as a separate bullet here.
...
\item
if the evaluation
is a precondition evaluation that
...
\item
if the evaluation
is a postcondition evaluation,
it is unspecified whether $E$ refers to
one of the temporary objects or the parameter object;
the choice is consistent within a complete evaluation
of a postcondition assertion.
\end{itemize}

\pnum
If an \grammarterm{id-expression} $E$
names a result binding $R$
and the implementation introduces any temporary objects
to hold the result object as specified in \ref{class.temporary},
then for any evaluation of $E$
that is sequenced before
the initialization of the result object denoted by $R$\iref{expr.call},
...
\pnum
The \defn{precondition evaluations}
and \defn{postcondition evaluations}
of a function call
consist of the evaluations of an
execution of a contract assertion $C$ for
every $C$ in
an evaluation sequence $X$ of the precondition assertions and
an evaluation sequence $Y$ of the postcondition assertions
of the function call.
Each such evaluation
is in the precondition evaluations
of the function call if
\begin{itemize}
\item
it is the evaluation of
the predicate of a precondition assertion,
\item
it is the occurrence of
a contract violation based on the result of
a precondition assertion, or
\item
it is the initialization of
a postcondition capture,
\end{itemize}
otherwise it is in the postcondition evaluations.
\begin{note}
An evaluation is in the postcondition evaluations if
\begin{itemize}
\item
it is the evaluation of
the predicate of a postcondition assertion,
\item
it is the occurrence of
a contract violation based on the result of
a postcondition assertion, or
\item
...
\pnum
The order of
the precondition evaluations and
the postcondition evaluations
of a function call
have the following constraints:
\begin{itemize}
\item
The evaluations associated with
a single execution of a contract assertion
are in the same order that they
are specified\iref{basic.contract.eval}.
\item
The evaluation of a predicate of a contract assertion
and the occurrence of a contract violation of a contract assertion
are in the same order as
the corresponding contract assertions
in $X$ or $Y$.
\item
The evaluation of the predicate
of a precondition assertion $A$
shall be after
the first evaluation of
the initialization of any postcondition capture
associated with a postcondition assertion $B$
for any $B$ in $Y$
that precedes $A$ in lexical order.
\item
The initialization of a postcondition capture
associated with a postcondition assertion $B$
shall be after
the first evaluation
associated with a precondition assertion $A$
for any $A$ in $X$
that precedes $B$ in lexical order.
\begin{note}
The evaluation of
the destruction of postcondition captures
happens in the reverse order
of their initialization,
but it can be interleaved
with the evaluations of
the predicates of other postcondition assertions.
\end{note}
\end{itemize}
...
and each precondition evaluation
of the function call occurs\iref{dcl.contract.func}.
...
sequenced before
the precondition evaluations of the function.
For any temporaries
...
all postcondition evaluations
of the function call occur\iref{dcl.contract.func}.
If the implementation introduces any temporary objects
to hold the result value as specified in \ref{class.temporary},
each postcondition evaluation
is indeterminately sequenced with respect to
