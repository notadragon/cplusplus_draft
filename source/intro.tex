...
except that introduced by evaluating a contract assertion with the
assume semantic
has an implicit precondition assertion\iref{basic.contract.implicit}
that it does not occur, although that assertion
may be evaluated with the assume
contract evaluation semantic\iref{basic.contract.eval}
and there is no guarantee that there is a way to
select any particular semantic.
\end{defnote}
