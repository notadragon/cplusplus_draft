...
\indexdefn{behavior!unconstrained}%
\definition{unconstrained behavior}{defns.unconstrained}
behavior for which this document
imposes no requirements

\begin{defnote}
Unconstrained behavior is expected whenever
a contract assertion with the assume semantic\iref{basic.contract.eval}
is violated
or control continues after the
guarding contract assertion\iref{basic.contract.implicit}
of undefined behavior.
\end{defnote}
\begin{defnote}
Permissible unconstrained behavior ranges
...
behavior that begins with
an implicit contract assertion
that the behavior cannot occur\iref{basic.contract.implicit}
and, if control continues after the contract assertion,
has unconstrained behavior.
...
Many incorrect program constructs do
not engender undefined behavior; they are required to be diagnosed.
There is no requirement that any particular semantic choice
be available for the implicit contract assertion.
\end{defnote}
...
%WORDINGNOTE: Many behaviors are described as resulting in undefined behavior,
% and when control continues past the guarding contract assertion of that
% behavior they then result in undefined behavior.
...
\pnum
\indextext{behavior!undefined}%
Certain operations are described in this document as
undefined behavior (for example, the effect of
attempting to modify a const object).
Such operations
evaluate a \defnadj{guarding}{contract assertion},
which is an implicit contract assertion
that they do not occur\iref{basic.contract.implicit}.
If control continues normally
after a guarding contract assertion,
the behavior is unconstrained.

\pnum
\indextext{behavior!unconstrained}%
Certain operations are described in this document as
unconstrained behavior (for example, control continuing
after evaluating the guarding contract assertion
associated with an instance of undefined behavior\iref{basic.contract.implicit}).
...
\begin{note}
Undefined behavior
whose guarding contract assertion
is evaluated with the ignore or assume semantic
does not imply an observable checkpoint.
\end{note}
...
for which for every unconstrained operation $U$
...
\indextext{behavior!undefined}%
If the selected execution contains an unconstrained operation,
